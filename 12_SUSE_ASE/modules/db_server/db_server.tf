...
resource "azurerm_virtual_machine" "db_server" {
  ...
  storage_data_disk {
    name              = "ASEshared-${element(var.db_server_hostnamelist, count.index)}"
    managed_disk_type = "Premium_LRS"
    create_option     = "Empty"
    lun               = 1
    disk_size_gb      = "64"
  ...
  storage_data_disk {
    name              = "ASEdata1-${element(var.db_server_hostnamelist, count.index)}"
    managed_disk_type = "Premium_LRS"
    create_option     = "Empty"
    lun               = 2
    disk_size_gb      = "64"
  ...
  storage_data_disk {
    name                      = "ASElog1-${element(var.db_server_hostnamelist, count.index)}"
    managed_disk_type         = "Premium_LRS"
    create_option             = "Empty"
    lun                       = 3
    disk_size_gb              = "64"
...
storage_data_disk {
  ...
  tags = "${merge(var.tags_map, map("Name", element(var.db_server_hostnamelist, count.index)), map("Environment", var.environment), map("Component", "SAP ASE Database"), map("Backup", var.backup))}"
}
